...
class m030_curlcurl( Scene ):
    def construct( self ):
        ...
        eq2[0].next_to( title, DOWN ).shift( 2.0 * LEFT + 2.0 * RIGHT )
        eq2[1].next_to( eq2[0], DOWN ).shift( 1.0 * DOWN - 2.0 * RIGHT )
        ...
        eq3[0].next_to( title, DOWN ).shift( 0.0 * LEFT )
        eq3[1].next_to( eq3[0], DOWN ).shift( 1.0 * DOWN - 2.0 * RIGHT )
        ...
        ref = title
        i = 2
        eq[i][0].next_to( ref, DOWN ).shift( 4.0 * DOWN + 5.0 * LEFT )
        ...
        eq2m[0].next_to( title, DOWN ).shift( 0.0 * LEFT )
        eq2m[1].next_to( eq2m[0], DOWN ).shift( 1.0 * DOWN - 2.0 * RIGHT )
        ...
        eq3m[0].next_to( title, DOWN ).shift( 0.0 * LEFT )
        eq3m[1].next_to( eq3m[0], DOWN ).shift( 1.0 * DOWN - 2.0 * RIGHT )
        ...
        g += VGroup( *eq[1] )
        g += VGroup( *eq[2] )
        self.play( g.animate.shift( 4.5 * RIGHT + 2.0 * UP ), run_time=3, rate_func=linear )
