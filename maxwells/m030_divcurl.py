...
class m030_divcurl( Scene ):
    def construct( self ):
        ...
        for i in range(2):
            sh = 1.00 * DOWN
            if i == 0:
                sh += 0.40 * DOWN + 0.20 * LEFT
            if i == 1:
                sh += 0.40 * DOWN + 0.60 * RIGHT
        ...
        eqE = eq[2].copy().move_to( eq[0] ).shift( 1.50 * RIGHT )
        eqH = eq[5].copy().move_to( eq[3] ).shift( 1.50 * RIGHT )

        self.play( ReplacementTransform( VGroup( *eq ), VGroup( eqE, eqH ) ) )
        self.wait( 5 )

        eqD = [ MathTex( r"\begin{bmatrix} \mathbf{E} \over \mathbf{H} \end{bmatrix} = { V \over m } { m \over A } = \Omega = \begin{bmatrix} \eta \end{bmatrix},\quad \eta = \sqrt{ \mu \over \epsilon }, \quad c = { 1 \over \sqrt{ \mu \epsilon } }" ),
                MathTex( r"\begin{bmatrix} \mathbf{E} \end{bmatrix} = \begin{bmatrix} \eta \mathbf{H} \end{bmatrix}" ) ]
        ...
        eq2 = [ MathTex( r"= { \rho \over \epsilon } - I  \mathbf{M} - I \sqrt{ \mu \over \epsilon } \sqrt{ \mu \epsilon } { { \partial \mathbf{H} } \over { \partial t } }" ),
                MathTex( r"= { \rho \over \epsilon } - I  \mathbf{M} - I {1 \over c } { { \partial (\eta \mathbf{H}) } \over { \partial t } }" ),
                MathTex( r"= { \rho \over \epsilon } - I  \mathbf{M} - {1 \over c } { { \partial (I \eta \mathbf{H}) } \over { \partial t } }" ) ]

        sh = 1.30 * DOWN + 1.00 * RIGHT
        write_aligned( self, eq[0], eq2[0], sh, None )
        self.wait( 5 )
        #sh = 1.30 * DOWN
        ...
        eq3 = [ MathTex( r" \boldsymbol{\nabla} (\eta \mathbf{H}) = \eta { \rho_{ \mathrm{m} } \over \mu } +  I \eta \mathbf{J}  + I \eta \epsilon { \partial  \mathbf{E}  \over \partial t } " ),
                #MathTex( r"= \sqrt{ \mu \over \epsilon } { \rho_{ \mathrm{m} } \over \mu } +  I \eta \mathbf{J}  + I \sqrt{ \mu \over \epsilon } \epsilon { \partial  \mathbf{E}  \over \partial t } " ),
                #MathTex( r"= { 1 \over \sqrt{ \mu \epsilon } } \rho_{ \mathrm{m} +  I \eta \mathbf{J}  + I \sqrt{ \mu \epsilon } { \partial  \mathbf{E}  \over \partial t } " ),
                MathTex( r"= c \rho_{ \mathrm{m} } +  I \eta \mathbf{J}  + I { 1 \over c } { \partial  \mathbf{E}  \over \partial t } " ),
                MathTex( r"\boldsymbol{\nabla} (I \eta \mathbf{H}) = I c \rho_{ \mathrm{m} } - \eta \mathbf{J}  - { 1 \over c } { \partial  \mathbf{E}  \over \partial t } " ) ]
        eq3[0].move_to( eqH ).shift( 1.30 * DOWN )
